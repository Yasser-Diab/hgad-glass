...
const migrationSource = readFileSync(
  new URL("../supabase/migrations/20260729154000_atomic_full_order_save.sql", import.meta.url),
  "utf8"
);
...
function canonicalAtomicSql(source) {
  const start = source.indexOf("create or replace function public.save_glass_order_atomic(");
...
test("migration and authoritative schema expose the same secured transaction", () => {
...
test("atomic RPC updates, prunes, and rolls back a foreign-row save as one transaction", async () => {
  ...
  const orderPayload = (id, orderNo, notes = "") => ({
    id,
    order_no: orderNo,
    document_id: null,
  ...
  try {
    ...
    await save(orderPayload(orderA, "GO-000001", "updated"), [
